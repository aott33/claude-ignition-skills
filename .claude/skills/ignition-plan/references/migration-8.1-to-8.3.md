...
| M3.4 **(security)** | **OPC UA:** trust the newly generated client and server certificates on third-party servers and clients. Anonymous OPC UA clients now get browse and read only; restore write/call only through explicit role mappings, and only with approval | Anonymous and authenticated clients tested for browse, read, write, call |
